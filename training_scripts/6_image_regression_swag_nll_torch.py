...
swag_model = SWAG(ResNetFineTuned,
  subspace_type="covariance", subspace_kwargs={'max_rank': 6})
...
scheduler = torch.optim.lr_scheduler.StepLR(optimizer, gamma = lr_gamma, step_size = 30, verbose = True)
...
for m in range(M):
  ...
  #  torch.save(swag_model.state_dict(), os.path.join(model_dir, "resnet50_nll.pth"))
   torch.save(swag_model, os.path.join(model_dir, "resnet50_swag"))
   model.eval()
